...
unsigned int Inode::Offset_To_Index(unsigned int offset)
{
    ...
    else if (block_cnt < DIRECT_PTR_NUM + SEC_PTR_NUM * PTR_IN_BLOCK_NUM)
    {
        ...
        Buffer::Read(i_addr[first_index] * BLOCK_SIZE, (char *)index_table, BLOCK_SIZE);
    ...
    else if (block_cnt < DIRECT_PTR_NUM + SEC_PTR_NUM * PTR_IN_BLOCK_NUM + TER_PTR_NUM * PTR_IN_BLOCK_NUM * PTR_IN_BLOCK_NUM)
    {
        ...
        Buffer::Read(second_block_no * BLOCK_SIZE, (char *)index_table, BLOCK_SIZE);
...
void Inode::Link(unsigned int physical_block_num)
{
    ...
    else if (block_cnt < DIRECT_PTR_NUM + SEC_PTR_NUM * PTR_IN_BLOCK_NUM)
    {
        ...
        Buffer::Read(i_addr[first_index] * BLOCK_SIZE, (char *)level2_index_table, BLOCK_SIZE);

        // change the index table
        level2_index_table[second_index] = physical_block_num;

        // store level2 index table
        Buffer::Write(i_addr[first_index] * BLOCK_SIZE, (char *)level2_index_table, BLOCK_SIZE);
    }
    else if (block_cnt < DIRECT_PTR_NUM + SEC_PTR_NUM * PTR_IN_BLOCK_NUM + TER_PTR_NUM * PTR_IN_BLOCK_NUM * PTR_IN_BLOCK_NUM)
    {
        // level 3
        ...
        Buffer::Read(level2_index_table[second_index] * BLOCK_SIZE, (char *)level3_index_table, BLOCK_SIZE);
        level3_index_table[third_index] = physical_block_num;
        Buffer::Write(level2_index_table[second_index] * BLOCK_SIZE, (char *)level3_index_table, BLOCK_SIZE);
    }
    else
    {
...
void Inode::Free_All_Space()
{
    ...
    if (block_cnt > DIRECT_PTR_NUM)
    {
        ...
        else
        {
            ...
            {
                unsigned int level2_index_table[PTR_IN_BLOCK_NUM];
                Buffer::Read(i_addr[DIRECT_PTR_NUM + SEC_PTR_NUM + i] * BLOCK_SIZE, (char *)level2_index_table, BLOCK_SIZE);
